...
# 【重要】ここをあなたのSupabaseの正確なテーブル名に書き換えてください
TABLE_WORDS = "physics_words"  # 単語クイズ用のテーブル名
TABLE_PROBLEMS = "electromagnetics"      # 画像で確認した例題用のテーブル名

# --- 2. データ取得関数 ---
def get_physics_data(mode, level_filter="すべて"):
    try:
        # モードによって参照するテーブルを切り替える
...
if df.empty:
    ...
    st.info(f"テーブル名 '{TABLE_WORDS if study_mode == '単語クイズ' else TABLE_PROBLEMS}' が正しいか確認してください。")
else:
    # モードが変わったらクイズをリセット
    ...
    if 'quiz' not in st.session_state:
        ...
        st.session_state.quiz = {
            "id": q['id'],
            "q_text": q['word'], # 例題テーブルではword列に「例題：...」が入っている
            "ans": q['mean'],
            "exp": q['explanation'],
            "options": options
        }
